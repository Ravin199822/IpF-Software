...
dict_extensions={
            ...
            'disc media_extensions'     :   ('.bin','.dmg','.iso','.toast','.vcd'),
            'database_extensions'       :   ('.csv','.dat','.db','.dbf','.log','.mdb','.sav','.sql','.tar','.xml','accdb'),
            ...
            'web_extensions'            :   ('.asp','.aspx','.cer','.cfm','.css','.htm','.html','.part','.rss','.xhtml'),
            'presentation_extensions'   :   ('.key','.odp','.pps','.ppt','.pptx'),
            'programming_extensions'    :   ('.c','.class','.cpp','.cs','.h','.java','.pl','.sh','.swift','.vb','.js','.jsp','.php','.py',),
            'spreadsheet_extensions'    :   ('.ods','.xls','.xlsm','.xlsx'),
            'system_extensions'         :   ('.bak','.cab','.cfg','.cpl','.cur','.dll','.dmp','.drv','.icns','.ini','.lnk','.msi','.sys','.tmp'),
            'video_extensions'          :   ('.3g2','.3gp','.avi','.flv','.h264','.m4v','.mkv','.mov','.mp4','.mpg','.mpeg','.rm','.swf','.vob','.wmv'),
            'file_extensions'           :   ('.doc','.docx','.odt','.rtf','.tex','.txt','.wpd','.wps','.pdf','pub')
}
...
loc_lable3=ttk.Label(page3_bydefault_lableframe,textvariable=files_path)
...
def createNewFolders():
    find_location=choose_location.get()
    folderpath=files_path.get()
    if find_location == 'custom':
        for extension_type, extension_tuple1 in dict_extensions.items():
            # print(f"{extension_type} : {extension_tuple1}")
            folder_name = extension_type.split('_')[0].title()+" Files"
            folder_path=os.path.join(folderpath, folder_name)
            for files in file_finder(folderpath, extension_tuple1):
    ...
    if find_location == 'loc':
        entered_location=add_page3_loc_var.get()
        # print(entered_location)
        for extension_type, extension_tuple1 in dict_extensions.items():
            folder_name = extension_type.split('_')[0].title()+" Files"
            fold_path=os.path.join(entered_location, folder_name)
            for files in file_finder(folderpath, extension_tuple1):
                if os.path.exists(fold_path):
                    ...
                    os.mkdir(fold_path)
                item_old_path=os.path.join(folderpath,files)
                item_new_path=os.path.join(fold_path,files)
                shutil.move(item_old_path,item_new_path)
    loc_lable3.grid_remove()
    add_page3_location.delete(0,tk.END)
...
def createNewWindow():
    ...
    if page3_radio_var1=='bydefault':              # user cliked on bydefault
        ...
        open_bydefault_btn=ttk.Button(page3_bydefault_lableframe, text="    Browse Files Location   ",style="C.TButton", command=browse_button )
